...
public class UserController {
    ...
    public ResponseEntity<ResponseDto> updateUserById(
    ...
    // ── Notification preferences (frontend — USER token) ──────────────────────

    @Operation(summary = "Get my notification preferences")
    @GetMapping("/preferences/notifications")
    @PreAuthorize("hasRole('USER')")
    public ResponseEntity<ResponseDto> getNotificationPreferences() {
        UUID userId = getCurrentUserId();
        return ResponseEntity.ok(
                ApiResponse.buildResponse(
                        userService.getNotificationPreferences(userId),
                        200,
    ...
    @PreAuthorize("hasRole('USER')")
    public ResponseEntity<ResponseDto> updateNotificationPreferences(
    ...
    // ── Notification preferences (scheduler — SERVICE token) ──────────────────

    @Operation(summary = "Get notification preferences by userId (service use)")
    @GetMapping("/{userId}/preferences/notifications")
    @PreAuthorize("hasAnyRole('USER', 'SERVICE')")
    public ResponseEntity<ResponseDto> getNotificationPreferencesByUserId(
